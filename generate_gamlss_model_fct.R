## This is a function to save gamlss models in a loop, each saved as an .rds file.
## Input args are vectors for family, mu terms, sigma terms, nu terms
## "long" is a flag to indicate whether the model is longitudinal
## "fixed_terms" is a flag for whether only the specified model should be generated,
## or if all combinations of variables should be saved.
## Combinations of variables will include terms in lower statistical moments, only if they are included in the higher statistical moments.
        ...
        for (j in 1:length(phenotype_set)) {
          ...
          if (fixed_terms == TRUE){
            ...
            if (long == TRUE){
              ...
              nu.terms <- nu_terms
              if(!is.null(tau_terms)){
                #tau.terms <- c(tau_terms, ph.t1) %>% {c(head(., 1), tail(., 1), .[-c(1, length(.))])}
                tau.terms <- tau_terms
              } else{tau.terms <- NULL}
            } else{
              mu.terms <- mu_terms
